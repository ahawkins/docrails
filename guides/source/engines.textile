...
Engines can connect to the rails configuration settings. These are the settings used in +application.rb+ and the files under +config/enviroment+. First we have to define an accessor on the config. The reader will point the +Blorg+ class.
...
module Blorgh
  class Engine < Rails::Engine
    ...
    config.blorgh = Blorgh
  end
end
</ruby>

Now, we can configure the +Blorg+ in +application.rb+

<ruby>
config.blorg.user_class = "User"
</ruby>
